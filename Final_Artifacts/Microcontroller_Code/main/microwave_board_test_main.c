        ...
        if (TD){ //No motion
            ...
            for(int i = 0; i < 10; i++){ //When motion is detected ask N times if a bird is visible
                ...
                if(bird){ //OpenMV Cam reports bird
                    ...
                    //play_bird_call();
                    dac_play_wav(dac_handle, (uint8_t *)audio_table, audio_size);
                    //play_sine_wave(); //Play sine wave on dac for 5s
                    //while(!gpio_get_level(PIN_TD)){} //Wait until motion sensor hold expires
